...
- `app/unauthorized.tsx` is a special auth-interrupt file, not a routable `/unauthorized` page, so direct redirects to `/unauthorized` produce a 404.
...
- Signed-in but unauthorized user:
  ...
  - The OAuth callback preserves the session and invokes `unauthorized()` instead of signing them out.
...
- The unauthorized and forbidden UI are special auth-interrupt files, not standalone routes
...
5. Sign in with a `@cars24.com` account that has no usable `org_members` access and confirm the unauthorized screen renders with a `401` response instead of a `404`.
6. Mark an existing `org_members` row inactive, sign in again, and confirm the unauthorized screen renders.
7. Sign in as a non-admin active member and open `/admin`; confirm you see the unauthorized screen.
8. While signed in and unauthorized, open `/login` and confirm the unauthorized screen renders instead of a `404`.
9. While signed in and unauthorized, use the sign-out button on the unauthorized screen and confirm you return to `/login`.
